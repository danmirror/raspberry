    ...
    try:
        ...
        while True:
            ...
            if(dist <1000 and minute == "00"): #kurang dari 1m dan minut itu no atau satu jam
                count +=1
                if(count==1):
                    send.send(selenoid,pump, dist) #kirim ke website
            else:
                count = 0
            ...
            if (hour== "07" ):
                ...
                print ("Jarak Air = %.2f mm" % dist)
                lcd.lcd_string("Jarak Air:", lcd.LCD_LINE_1)
                lcd.lcd_string(" " + str(format(dist, '.2f')) + "mm" +" ->on", lcd.LCD_LINE_2)
                time.sleep(1)
            else:
                relay.pump_high()
                relay.selenoid_high()
                ...
                lcd.lcd_string("Jarak Air:", lcd.LCD_LINE_1)
                lcd.lcd_string(" " + str(format(dist, '.2f')) + "mm" +" ->off", lcd.LCD_LINE_2)
